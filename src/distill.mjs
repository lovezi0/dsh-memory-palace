...
export function createDistill({ ctx, getConfig, paths, records, state }) {
  ...
  async function distillSessionCore(session, dirs, fromSeq, opts) {
    ...
    const t0 = Date.now();
    dbg("session request", { provider, model, messages: messages.length, maxTokens: c.summaryMaxTokens, timeoutMs, maxRetries: RETRY_CONSTANTS.MAX_RETRIES });
    let result;
    try {
      result = await runWithRetry(
        () => streamOnce({ provider, model, system: SUMMARY_PROMPT({ allowDelete }), messages, maxTokens: c.summaryMaxTokens }, timeoutMs),
        {
  ...
  async function distillProjectMemory(cwd, session) {
    ...
    try {
      ...
      dbg("project request", { provider, model, inputChars: raw.length, maxTokens: c.projectMaxTokens, timeoutMs: c.summaryTimeoutMs || 60000 });
