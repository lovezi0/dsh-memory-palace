...
export function createPaths(getConfig, getCwd = () => null) {
  ...
  // 读取源：dsh 原生目录恒在首位（不设 existsSync 门槛——即便每日日志目录尚未创建，
  // 也要能读到同级的 .deepseek-harness/MEMORY.md），其后叠加磁盘上已存在的 buddy 目录。
  // v1.7.0 修复：旧实现是「目录级排他」——buddyDirs() 命中即 return b，导致项目同时存在
  // .workbuddy/memory（哪怕只有日志、无 MEMORY.md）时，.deepseek-harness/MEMORY.md 既读不到
  // 也写不进、长期静默失效。现改为「dsh 优先 + buddy 叠加」，两者都可被读到。
  // 顺序定义：[.deepseek-harness > .workbuddy > .codebuddy]
  function readDirs(cwd = activeCwd()) {
    ...
    if (d) out.push(d);
    out.push(...buddyDirs(cwd));
    return out;
  }
  // 写入目标：存在 buddy 目录则全部同步写入；否则回退 dsh 目录（按需创建）。
  // 写入优先级保持 buddy 优先不变（读 dsh 优先 / 写 buddy 优先，读写刻意不对称）。
