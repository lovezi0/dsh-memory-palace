...
// 本地日期偏移（0=今天，1=昨天，2=前天…），供 memory_read 按**日期**取日志
// （而不是"最近 N 份文件"——跨日跳空时会取偏）。同 todayISO 的教训：必须本地时间。
export const isoDaysAgo = (n = 0) => {
  const d = new Date();
...
export function budgetClip(text, budget) {
  ...
  // v1.8.0-alpha.1 修复（D2）：中部结构行不再随内容一起丢弃——旧实现把中部的 `## 章节标题`
  // 当普通内容行处理，落在尾部窗口外即消失，而截断标记却自称「仅注入结构行与尾部最近条目」
  // （声明与行为不符，实测中部 3 个章节标题整体丢失）。现把窗口外幸存的结构行按原序
  // 列在标记之后，保证章节大纲始终完整可见。结构行不计预算（大纲优先于条目）。
  const middleStructural = lines.slice(i, j + 1).filter((l) => isStructural(l.trim()));
  const marker = [`…（已截断，原 ${text.length} 字符，仅注入结构行与尾部最近条目）`, ...middleStructural];
...
// 故需单独扫描工具结果文本。命中即视为错误信号（作为 isError 传给记忆子代理）。
