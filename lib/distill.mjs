...
import { expandHome, readMdSync, toHomeShort, nowStamp, stripSmartTag } from "./common/text.mjs";
...
export function createDistill({ ctx, getConfig, paths, records, state }) {
  ...
  async function distillSessionCore(session, dirs, fromSeq, opts) {
    ...
    const newEvents = eventsFrom(session, fromSeq).filter((e) => SURFACE.has(e.type));
    ...
    if (!hist.length) {
      dbgFail("no events in range", { fromSeq, eventsLen: eventsFrom(session).length, newEvents: newEvents.length, hasDerive: !!session.deriveEventMessage });
      return { ok: false };
    }
    ...
    for (const d of durable) {
      ...
      let fact = String(d.fact ?? "").trim();
      if (!fact) continue;
      // v1.4.1：写侧不再拼接 [smart] 标签（防回喂二次拼接形成 [smart] [smart] 双前缀、
      // 防标签注意力稀释）；防御性剥除 LLM 自行带回的标签，保证落盘条目干净。
      fact = stripSmartTag(fact).trim();
      const line = `- ${fact}`;
      if (d.scope === "user") {
