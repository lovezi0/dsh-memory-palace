...
//   顺带完成一次「刷新到最新」。
// - v1.7.2 起**静默会话整体不投影**（isSilent(session)）：宿主「模式」是 agent 平面预设，
//   本插件却在 host 平面，投影对每个 agent 都生效——官方 minimal 预设（persona complete:true
//   独占 system prompt 的"裸测环境"）下必须自行按会话静默，否则记忆正文仍会常驻历史吃 token。
//
...
// 投影消息的 source 标注。
// v1.7.2-alpha.2（dsh 0.1.7 适配）：宿主删除了通用 catch-all `plugin` kind（MessageSourceMap 不再含
...
// v1.7.2-alpha.2：新消息 kind='plugin:dsh-memory-palace'；同时**保留旧 kind='plugin'+plugin 字段的识别**
 ...
 * v1.7.2：新增 `session` + `isSilent` —— 静默会话（官方 minimal 等预设）整体不投影。
 * @param {{ getConfig: () => object, paths: object, session?: object, isSilent?: (session?: object) => boolean }} deps
 * @returns {object[]} 按 [用户级, 项目级…, 今日日志…] 顺序的消息数组（可能为空）
 */
export function buildProjections({ getConfig, paths, session, isSilent }) {
  const cfg = getConfig();
  if (!cfg.enabled) return [];
  // v1.7.2：静默会话（极简模式等）不投影——投影消息是**常驻历史**，比 section 更"重"：
  // 它要一路吃 token 直到 compaction，正是"裸测环境"最不该有的东西。
  if (isSilent?.(session)) return [];
  const out = [];
  // 用户级 MEMORY.md
  const userFile = expandHome(cfg.userMemoryPath);
  const userText = readMdSync(userFile);
  if (userText) {
    // v1.8.0-alpha.1 修复（C1）：先剥墓碑/标签再裁剪——旧顺序 budgetClip 在内层，墓碑先消耗
    // 预算再被外层剥掉 = 白白挤掉有效条目。三处（用户级/项目级/今日日志）同此顺序。
    const clipped = budgetClip(stripDeletedLines(stripSmartTag(userText)), cfg.userBudgetChars);
    const msg = buildProjection(clipped, `# 用户级记忆 (${toHomeShort(userFile)})`);
    if (msg) out.push(msg);
  }
  // 项目级 MEMORY.md：按 readDirs 顺序（dsh 优先叠加 buddy），同一文件路径只取一次。
  // 显式透传当前会话 cwd，避免依赖闭包 activeCwd 造成多会话并发/切换工作区时记忆串台
  const seen = new Set();
  const cwd = session?.header?.cwd;
  for (const dir of paths.readDirs(cwd)) {
    for (const file of paths.memoryReadCandidates(dir, cwd)) {
      if (seen.has(file)) continue;
      seen.add(file);
        const text = readMdSync(file);
        if (!text) continue;
        const clipped = budgetClip(stripDeletedLines(stripSmartTag(text)), cfg.workspaceBudgetChars);
...
export function registerProjection({ ctx, getConfig, paths, isSilent }) {
  ctx.on("agent/pre-step", async ({ agent, messages, step, signal }, next) => {
    ...
    try {
      ...
      // v1.7.2：静默会话（极简模式等）不投影。放在 next() 之后、构造之前，成本最低。
      if (isSilent?.(session)) return decision;
      const desired = buildProjections({ getConfig, paths, session, isSilent });
