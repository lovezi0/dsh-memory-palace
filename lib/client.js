window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const zh = {
      ...
      memoryModeHintHybrid: "混合模式：记忆子代理自动维护今日日志（章节化、标删去重），agent 主动维护 MEMORY.md（章节化写入、整章节替换、门禁内全量重整）。切换需重启 dsh 生效。",
    ...
    const en = {
      ...
      memoryModeHintHybrid: "Hybrid mode: a memory sub-agent auto-maintains today's daily log (chaptered, dedupe by strikethrough); the agent proactively maintains MEMORY.md (chaptered writes, section replace, gated full reorganize). Switching requires a dsh restart.",
