window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const zh = {
      ...
      summarizeHint: "三种记忆模式的共用总闸门：关闭后三种模式都不写记忆。插件模式下它作为「agent 主动记忆」主路径失效时的安全网，只留原始文本、不做总结；智能模式走 LLM 智能摘要（总结 + 提炼）；混合模式下整个记忆子代理停摆（不做兜底写入）。",
      ...
      memoryModeHintHybrid: "混合模式（默认）：记忆子代理自动维护今日日志（章节化、标删去重），agent 主动维护 MEMORY.md（章节化写入、整章节替换、门禁内全量重整）。切换需重启 dsh 生效。",
    ...
    const en = {
      ...
      summarizeHint: "The shared master gate for all three memory modes: when off, none of them writes any memory. In plugin mode it is the safety net when the 'agent proactive memory' main path fails (keeps raw text, no summarization); in smart mode it enables the LLM summarization path (summary + extraction); in hybrid mode the whole memory sub-agent stays idle (no fallback entry either).",
      ...
      memoryModeHintHybrid: "Hybrid mode (default): a memory sub-agent auto-maintains today's daily log (chaptered, dedupe by strikethrough); the agent proactively maintains MEMORY.md (chaptered writes, section replace, gated full reorganize). Switching requires a dsh restart.",
