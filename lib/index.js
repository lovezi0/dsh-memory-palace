...
export const Config = Schema.object({
  ...
  // ---- v1.4.0：智能模式最大输出 Token 限制（特性1，默认保守，可由设置页放大以防摘要/durable 被截断） ----
  summaryMaxTokens: Schema.number().default(2000).description("智能模式会话摘要 LLM 的最大输出 Token（summary + durable 合计上限）。默认 2000，过小会截断 durable 事实；可上调到 4000+。"),
  projectMaxTokens: Schema.number().default(8000).description("手动「蒸馏项目记忆」LLM 的最大输出 Token（蒸馏后 MEMORY.md 长度上限）。默认 8000，过小会截断浓缩结果；可上调。"),
  // ---- v1.4.0：蒸馏时回喂存量记忆（特性3，默认关；开启后自动智能模式 turn/end 与手动蒸馏按钮均回喂，delete 仅手动放开） ----
  feedbackEnabled: Schema.boolean().default(false).description("蒸馏时把项目级 + 用户级 MEMORY.md 全文（逐行编号、无截断）回喂给 LLM，使其能基于既有记忆做 add/replace/delete 增量维护。默认关；开启后自动智能模式（turn/end）与手动蒸馏会话按钮均回喂存量记忆；delete 仍仅手动按钮开放，自动模式跳过（防误删）。"),
});
