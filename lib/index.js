...
export function apply(ctx, config) {
  ...
  // 尚未挂载（installSettingsSection 的 setSource 在 ctx.inject(["settings"]) 回调里才替换），
  // 此刻 source() 读的是 cordis 组合配置（默认 plugin），条件注册会导致设置页切 hybrid 后
  // 工具/子代理从未注册（v1.6.0 实装踩坑：settings.yaml memoryMode=hybrid 但 registerHybrid
  // 未执行 → 子代理 TypeError 被吞、日志全空）。
