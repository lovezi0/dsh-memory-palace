...
export function apply(ctx, config) {
  ...
  const state = {
    ...
    sawErrorTurn: false,      // 跨 turn 累积的 request 级错误信号（作为 isError 传给记忆子代理）
    ...
    // v1.8.0：recentAgentWrote（原防闲聊闸门信号）与 lastSummarizedSeq / summarySessionId
    // （原 smart 模式增量断点）随 plugin / smart 两模式一并删除。
  };
  ...
  ctx.on("session/event", (session, event) => {
    ...
    if (type === "user/message" || type === "assistant/message" || type === "tool/result") {
      ...
      // 会被采集进 buffer，并触发下方 `type === "user/message" && state.turnBuffer.length` 的
