...
// v1.8.0：适配 dsh 0.1.7-alpha.1 契约破坏（DSH-CONTRACT-TRACKING.md §1.6）。
...
export const Config = Schema.object({
  enabled: Schema.boolean().default(true).description("启用 memory-palace 记忆注入与每日日志写入。").volatile(),
  ...
  dailyLogRetentionDays: Schema.number().default(30).description("每日日志保留天数，过期日志会被蒸馏进 MEMORY.md。").volatile(),
  userBudgetChars: Schema.number().default(4000).description("注入系统提示词的用户级记忆长度上限（字符）。").volatile(),
  workspaceBudgetChars: Schema.number().default(3000).description("注入系统提示词的工作区级记忆长度上限（字符）。").volatile(),
  ...
  summarize: Schema.boolean().default(true).description("它是「agent 主动记忆」主路径失效时的安全网，保证实质工作不丢，代价是只留原始文本、不做总结。").volatile(),
  autoCaptureErrors: Schema.boolean().default(true).description("对话出错时（含代码运行报错、工具执行失败）自动把『错误现象』写入对应 MEMORY.md（『根因/方案』由 agent 按记忆公民指令场景①主动记；默认开，关闭无需重启）。").volatile(),
  // ---- v1.1.0：记忆模式（plugin=记忆公民指令+轮次轻量+错误捕获；smart=LLM 智能会话摘要）----
  // v1.6.0：+hybrid（子 agent 自动维护日志 + agent 主动维护 MEMORY.md），三档并存，切换需重启 dsh。
  memoryMode: Schema.union(["plugin", "smart", "hybrid"]).default("hybrid").description("记忆模式（默认 hybrid）：plugin=记忆公民指令+轮次轻量+错误捕获；smart=LLM 智能会话摘要（summary→每日日志 + durable→MEMORY.md）；hybrid=记忆子代理自动维护今日日志（标删去重）+ agent 主动维护 MEMORY.md（章节化写入/整章节替换/门禁内全量重整）。切换需重启 dsh 生效。").volatile(),
  summaryModel: Schema.string().default("").description("记忆插件当前使用的模型（智能模式会话摘要 / 手动蒸馏 / hybrid 记忆子代理共用）。留空=复用当前会话 provider/model；也可填 provider/model（如 deepseek/deepseek-chat）固定廉价模型省 token。").volatile(),
  summaryTimeoutMs: Schema.number().default(60000).description("蒸馏 LLM 调用的超时（毫秒），超时视为失败并降级；覆盖智能模式摘要与手动蒸馏两条链路；默认 60000（60s）。").volatile(),
  ...
  summaryMaxTokens: Schema.number().default(2000).description("会话摘要 LLM 的最终输出软预算（prompt 约束，思考不受限；实际硬上限由模型自身 maxTokens 决定）。默认 2000；可调大以容纳更多 durable 事实。").volatile(),
  ...
  feedbackEnabled: Schema.boolean().default(false).description("每次蒸馏时把项目级 + 用户级 MEMORY.md 全文（逐行编号、无截断）回喂给 LLM，使其能基于既有记忆做增量维护。delete 仍仅手动蒸馏按钮开放，自动模式跳过（防误删）。hybrid 模式下仅手动触发蒸馏受影响（自动路径走记忆子代理，自带日志回喂）。默认关。").volatile(),
  // ---- v1.6.0：hybrid 模式配置（仅 memoryMode=hybrid 时生效） ----
  reorgCooldownDays: Schema.number().default(7).description("hybrid 模式：项目级 MEMORY.md 全量重整的冷却天数（距上次重整）。与「超出注入预算」双条件同时满足才允许 memory_reorganize；时间戳以 HTML 注释落在 MEMORY.md 文件尾。").volatile(),
  subagentLogBudget: Schema.number().default(20000).description("hybrid 模式：记忆子代理回喂今日工作日志的字符上限。超出时仅回喂章节目录，子代理用 log_read_section 按需读取章节。").volatile(),
  // ---- v1.7.1（特性3）：自定义指令（用户自写，经 system prompt 注入到「记忆分工 prompt」之后） ----
  // 放 system 而非 E 投影：用户配置一次即恒定 → 零缓存成本（见下方 section 的分流原则）。
  customInstructions: Schema.string().default("").description("自定义指令：非空时追加到系统提示词「记忆分工说明」之后，用于承载不适合写进用户记忆的特殊指令。对全部会话与三种记忆模式生效；留空则不注入。").volatile(),
});

export const inject = ["systemPrompt", "tools", "llm", "webServer", "webRuntime"];

/**
 * v1.8.0（dsh 0.1.7 适配）：解包 cordis 解析后的 cosmokit Volatile 引用。
...
export function apply(ctx, config) {
  ...
  // v1.8.0：配置真源统一 = cordis 注入的 config（0.1.7 起即 profile 条目 config、热改原地生效）。
  let source = () => unwrapConfig(config);
  const baseEntry = { ...config };

  // ---- v1.1.3：设置读写 route（真保存）/ v1.8.0：适配 dsh 0.1.7 SettingsForms ----
  ...
  // v1.8.0 适配（dsh 0.1.7-alpha.1）：dsh-settings 整包重写为 SettingsForms——
  ...
  ctx.inject(["settings"], (sctx) => {
    ...
    // v1.8.0 双栈：旧宿主（0.1.6 线 SettingsProvider）有 installSection，须注册节并把 setSource
  ...
  // 回调里注册并同步 setSource），此刻 source() 读的是 cordis 组合配置（未显式配置时即 schema 默认），
  ...
  async function _settle(capturedTurn, isError, errMsg) {
    ...
    if (cfg.autoCaptureErrors && effectiveIsError) {
      ...
      void records.captureError(capturedTurn, { kind: "error", message: finalErrMsg }, dirs, cwd).catch(() => {});
    }
